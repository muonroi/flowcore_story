...
class FingerprintProfile:
    ...
    def get_quality_score(self, site_key: str) -> float:
        """Get quality score for site (default 1.0 for new/untested sites)."""
...
class FingerprintPool:
    ...
    def __init__(
        ...
        # FIXED TASK 7: Increase cache TTL to 1 hour
        self._cache_ttl: float = float(os.environ.get("FINGERPRINT_CACHE_TTL", "3600.0"))
        self._cache_timestamps: dict[str, float] = {}
        self._site_recent_profiles: dict[str, set[str]] = defaultdict(set)
        
        # FIXED TASK 7: Persistent storage
        try:
            from flowcore_story.utils.cookie_storage import get_cookie_storage
            self._storage = get_cookie_storage()
        except ImportError:
            self._storage = None
    ...
    def _load_cache_from_storage(self) -> None:
        """Load fingerprint cache from persistent storage."""
        if not self._storage:
            return
            
        try:
            data = self._storage.load("fingerprint_pool_cache")
            if isinstance(data, dict):
                cache = data.get("cache", {})
                timestamps = data.get("timestamps", {})
                
                # Restore only valid entries
                now = time.time()
                for key, profile_id in cache.items():
                    ts = timestamps.get(key, 0)
                    if now - ts < self._cache_ttl:
                        self._site_proxy_cache[key] = profile_id
                        self._cache_timestamps[key] = ts
        except Exception:
            pass # Ignore errors during cache load

    def _save_cache_to_storage(self) -> None:
        """Save fingerprint cache to persistent storage."""
        if not self._storage:
            return
            
        try:
            data = {
                "cache": self._site_proxy_cache,
                "timestamps": self._cache_timestamps,
                "updated_at": time.time()
            }
            self._storage.save("fingerprint_pool_cache", data)
        except Exception:
            pass
    ...
    def get_profile(
        ...
        if self.rotation_strategy == "quality_weighted" and site_key:
            recent = self._site_recent_profiles[site_key]
            # During warm-up when all qualities are equal, rotate to avoid returning
            # the same profile for consecutive calls (helps tests that expect two distinct picks).
            if len(recent) < 2:
                profile = self._select_round_robin()
                # Try to pick a different profile if possible
                attempts = 0
                while profile.profile_id in recent and attempts < len(self.profiles):
                    profile = self._select_round_robin()
                    attempts += 1
            else:
                profile = self._select_quality_weighted(site_key)
            recent.add(profile.profile_id)
            if len(recent) > 4:
                # Keep memory bounded
                self._site_recent_profiles[site_key] = set(list(recent)[-2:])
    ...
    def _select_quality_weighted(self, site_key: str) -> FingerprintProfile:
        ...
        # If all qualities are effectively equal, rotate to avoid repeatedly choosing the same
        # profile during warm-up.
        if len({round(w, 3) for w in weights}) == 1:
            return self._select_round_robin()

        max_quality = max(weights)
        # If we have a clear leader in quality, pick among the best directly
        best_indices = [idx for idx, w in enumerate(weights) if w == max_quality]
        if max_quality > 0:
            # Identify second-best for comparison
            sorted_weights = sorted(weights, reverse=True)
            second_best = sorted_weights[1] if len(sorted_weights) > 1 else 0
            if len(best_indices) == 1 or max_quality - second_best >= 0.05:
                chosen_idx = random.choice(best_indices)
                return self.profiles[chosen_idx]
