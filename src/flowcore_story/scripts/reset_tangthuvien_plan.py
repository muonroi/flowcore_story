...
def reset_tangthuvien():
    print("=== RESETTING PLAN FOR: TANGTHUVIEN ===")
    try:
        engine = create_engine(DATABASE_URL)
        with engine.connect() as conn:
            # 1. Clean Story Queue (Pending/Processing only)
            # We keep 'completed' or 'error' to avoid re-crawling finished work if possible,
            # but for a fresh plan, cleaning pending is crucial.
            ...
            print("\nSUCCESS: TangThuVien is ready to be re-planned with new Adapter logic!")
